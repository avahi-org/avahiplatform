...
class productDescriptionGeneration:
    ...
    def model_invoke(self, prompt, model_name=None, max_retries=15, initial_delay=2):
        ...
        while retries < max_retries:
            try:
                logger.info("Invoking model to for Product Description Generation.")
    ...
    def generate_product_description(self, product_sku, event_name, customer_segmentation, user_prompt=None, model_name=None):
        if not product_sku and not event_name and not customer_segmentation:
            logger.error("Missing input: Please ensure you provide the event name, and customer segmentation to generate the product description.")
            raise ValueError("Incomplete input provided. Please ensure you provide the event name, and customer segmentation to generate the product description.")

        if user_prompt:
            prompt = f"""
                    Generate a product description for the following parameters:
                    - Product SKU: {product_sku}
                    - Event: {event_name}
                    - Customer Segment: {customer_segmentation}
                    {user_prompt}
