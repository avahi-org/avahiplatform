...
class BedrockGrammarCorrection:
    ...
    def model_invoke(self, prompt, model_name=None, max_retries=15, initial_delay=2):
        ...
        while retries < max_retries:
            try:
                logger.info("Invoking model to rectify grammar and spelling of the content.")
    ...
    def grammar_correction_text(self, text, user_prompt=None, model_name=None):
        if not text:
            logger.error("Input text cannot be empty.")
            raise ValueError("Input text cannot be empty.")

        prompt = user_prompt if user_prompt else f"Please rectify grammar and spelling of the following text: {text}"
    ...
    def grammar_correction_file(self, file_path, user_prompt=None, model_name=None):
        ...
        return self.grammar_correction_text(text, user_prompt, model_name)

    def grammar_correction_s3_file(self, s3_file_path, user_prompt=None, model_name=None):
        s3 = boto3.client('s3')
